...
这不是当前官方 Codex Pet manifest 的必需字段。现在 Codex 本地宠物主要读取 `window-possum/pet.json` 和 `window-possum/spritesheet.webp`，所以我把台词做成 sidecar 文件，方便未来 Codex/OpenPets 支持自定义气泡，或者你自己做 overlay/fork 时直接接入。

台词主打“打工人低功耗自救”和“背手负鼠式精神离职”，比如：
...
`speech/zh-CN.json` 是可选台词包，不影响 Codex Pet 的标准安装。
